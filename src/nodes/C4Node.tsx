...
import type { C4Node as C4NodeType, C4NodeData, C4Kind, Hint, Attachment } from "../types";
...
/** Render operation names as draggable slot chips with status dots */
  ...
  const chips = members.map((fn) => {
    ...
    return (
      <div
        ...
        style={sc ? { borderColor: dimmed ? sc.hex + "40" : sc.hex + "99" } : undefined}
        onDragStart={(e: DragEvent) => {
          e.dataTransfer.setData("text/plain", fn.id);
      ...
      >
        <Code size={10} className="shrink-0 opacity-50" />
    ...
    <div className="w-full px-2 py-1">
      <div className="flex gap-1 overflow-hidden">
        ...
        {attachments.length > 3 && (
          <div className="flex items-center justify-center rounded border border-zinc-200 dark:border-zinc-700 bg-zinc-100 dark:bg-zinc-800 text-[9px] text-zinc-400 dark:text-zinc-500"
            style={{ width: 52, height: 36 }}
          >
            +{attachments.length - 3}
          </div>
...
export function C4Node({ id, data, selected }: NodeProps<C4NodeType>) {
  ...
  const models = data._models as
    | { id: string; name: string; status?: string }[]
    | undefined;
  const hasMembers = (members && members.length > 0) || (processes && processes.length > 0) || (models && models.length > 0);
        ...
        <div className="relative h-[160px]">
          ...
          >
            ...
            <div className="w-full text-center text-sm font-semibold leading-tight break-all">
              <span className="text-zinc-400 dark:text-zinc-500">{data._relationships?.[0]?.direction === "out" ? "\u2190" : "\u2192"}</span> {data.name}
            </div>
            {data.technology && (
              ...
              {models && models.map((m) => {
                ...
                return (
                  <div
                    ...
                    style={sc ? { borderColor: sc.hex + "99" } : undefined}
                  >
                    <Table size={10} className="shrink-0 opacity-50" />
                    <span className="truncate">{m.name || "Untitled"}</span>
                  </div>
                );
              })}
              {members && <OperationList members={members} inline />}
            </div>
          </div>
        )}
      </div>
  ...
  if (data.kind === "person") {
    ...
    const silhouetteFill = selected ? (isDark ? "#a1a1aa" : "#27272a") : "#52525b";
        ...
        <ShapeBackground
          ...
          strokeClass={
            selected
              ...
              : statusColor
                ? (groupHighlight ? statusColor.strokeClass : statusColor.dimStrokeClass)
                : groupHighlight
                  ? "stroke-zinc-400 dark:stroke-zinc-500"
      ...
      {isComponent && hasMembers && (
        <div className="w-full px-2 py-1.5">
          <div className="flex flex-col gap-1">
            ...
            {/* Ungrouped operation chips */}
            {members && <OperationList members={members} inline />}
          </div>
        </div>
      )}
      {(data.kind === "container" || data.kind === "component") && <AttachmentStrip attachments={attachments} />}
    </div>
  );
}
