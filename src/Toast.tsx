      ...
      {toasts.length > 0 && (
        <div className="fixed bottom-12 right-4 z-50 flex flex-col gap-1.5 max-w-[280px]">
          {toasts.map((t) => (
            <div
              key={t.id}
              className="flex items-center gap-1.5 rounded border border-[var(--border-overlay)] bg-[var(--surface-overlay)] backdrop-blur-md px-2.5 py-1.5 text-xs shadow-sm cursor-pointer animate-in slide-in-from-bottom-2"
