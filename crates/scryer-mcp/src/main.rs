...
fn init_project() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if wrote_any {
        ...
        if has_claude {
            ...
            eprintln!("  \"permissions\": {{ \"allow\": [\"mcp__scryer__read_model\", \"mcp__scryer__search_model\", \"mcp__scryer__get_rules\", \"mcp__scryer__get_unimplemented\", \"mcp__scryer__read_codebase\", \"mcp__scryer__validate_model\"] }}");
        }
    }
