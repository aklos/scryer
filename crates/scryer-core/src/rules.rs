...
One obligation is NEVER waived, because it is cheap only in the moment: if you changed the behaviour of an anchored symbol, confirm or reword its claims BEFORE you finish, while the diff is still in your context — `locate {file, symbol}` returns just those claims, a few lines to check. Deferred, the same reconciliation costs a later session thousands of tokens to reconstruct."#,
    ...
    Rule {
        id: 52,
        slug: "styles",
        title: "A container declares the style its code actually has; components then carry its layers",
        tags: &[
            "style", "layer", "hexagonal", "feature-sliced", "core-shell", "pipeline",
            "structural", "violation", "conformance",
            "architecture", "import", "dependency", "placement", "path", "domain",
            "application", "infrastructure", "presentation",
        ],
        body: r#"A style is the model's horizontal axis — the fixed vocabulary that says what KIND of thing a container is and what ROLE each component plays inside it, the way EARS fixes a claim's shape and a concern fixes a slug's meaning. Declare a style only when the code ACTUALLY follows one — its directories, imports and entry points already have that shape: `hexagonal` for services, backends and library cores (layers presentation, infrastructure, application, domain); `feature-sliced` for SPAs, docs and static sites (app, pages, widgets, features, entities, shared); `core-shell` for CLIs, small libraries and script repos (shell, core); `pipeline` for ETL and dbt work (source, staging, intermediate, marts). When modeling an existing codebase, NEVER guess a style to make the map look organised: a container whose code has no recognisable shape stays unstyled, its components carry no layer, and the model is a faithful description of the disorder. Deciding what style such a container SHOULD take — and moving the code to match — is a refactoring decision for a later, deliberate pass, with the user. Under a styled container every component carries one of the style's layers (the tools reject anything else); symbols inherit their component's layer.
The style is enforced, not advised: `add_links` rejects a link the layer matrix forbids (in hexagonal: domain depends on nothing outside domain; application on domain; infrastructure on application and domain; presentation on application; never presentation ↔ infrastructure), a same-layer link between two components must be `kind: uses`, and a link entering a container from outside lands on its inbound layer (presentation or application; app or pages; shell). Health then checks the CODE the same way from the real import graph — a domain file importing an infrastructure file, a banned package (domain importing React or sqlx), a file sitting on another layer's path. These are STRUCTURAL VIOLATIONS: every one is a real import or file to fix, never a judgment call.
...
"Fix the structural violations" means change the CODE until the import graph fits the declared style. List them with `get_health {nodeId}` on the container (the top-level report only samples and counts per container) or `scryer-mcp check`; each line names the file and the import or misplaced path. Fix at the source named: invert the dependency through a port the inner layer owns, move the file to its layer's directory, replace the banned package with an adapter. What is NOT a fix: removing the container's `style` or a component's `layer`, deleting or re-kinding the link, or relabelling a component to whichever layer makes the import legal — a label may change only when the component genuinely is that layer, and then its whole file set moves with it. Re-run the check after each move; moved files re-anchor through `reconcile_drift`. A container with NO style has no structural violations to fix — declaring one ("make Model Core hexagonal") is a separate, user-approved decision: declare the style, give every component its layer from what the code IS, and only then does the check name what has to move."#,
    },
];
