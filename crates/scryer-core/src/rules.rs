...
Statements also carry a markdown-lite display markup the UI renders — and strips for comparison, search, and word-diffing, so it is presentation, never meaning. Wrap the scan anchors — the grammar keywords (including "then") and the response verb — in **bold**, and NOTHING else: "**When** a send-failure status callback arrives, **append** a failed-send event". A ubiquitous claim bolds its leading verb: "**Authenticate** every inbound POST". Never wrap the condition clause or any other run of text in markers — the keyword and its comma already delimit the clause. Markup belongs in responsibility statements ONLY — never in directives, descriptions, names, or properties."#,
    ...
    Rule {
        ...
        body: r#"Author the intended change into the model BEFORE writing code: add/extend the nodes, responsibilities, and links it implies, at the right altitude ([[altitude]]), with the intent tools (`add_person` / `add_system` / `add_container` / `add_component` / `add_symbol`, `update_nodes`, `add_links`, …). These write the PLAN — a draft on the user's canvas — not code ([[model-layers]]). If the change conflicts with an existing responsibility or directive, surface it; don't silently diverge. When other work may share the plan (parallel sessions, a dev on the canvas), first name YOUR task: `set_change {rationale}` opens a named change and your plan writes tag to it automatically, keeping workstreams separable — resume a prior session's change with `set_change {change_id}` (listed in `get_pending`'s `openChanges`) ([[change-ledger]]). Quick serial edits can skip this and go unfiled. Only changes that alter what the model claims need a plan entry at all ([[proportionality]])."#,
    },
    Rule {
        id: 25,
        slug: "loop-sign-off",
    ...
    Rule {
        ...
        body: r#"`mark_implemented` what you built (folds it from the plan into the committed model), passing `anchors` and `tests` so fold + anchor + attach is one atomic call, and `flag_drift` anything the code does that the plan didn't capture. The fold is gated on evidence ([[fold-evidence-gate]]); claims reworded or added after sign-off wait for the developer ([[fold-after-sign-off]]); you may fold a node in layers ([[fold-in-layers]]); the response ends with a scoped post-flight to act on ([[fold-post-flight]]). If you opened a change, `mark_implemented {change}` folds exactly its entries; when the last one folds the change closes and its rationale is recorded in the history log. Then VERIFY: `get_test_radius` names the test files your change invalidated (missing or stale verdicts) — run exactly those with the runner's JUnit reporter on and `ingest_test_report` each report file, so the claims' verdicts are current before you move on ([[test-verdicts]]). Then reconcile and continue: `reconcile_drift` advances the anchor once the scope is clean, and the next responsibility starts the loop again at BUILD."#,
    },
    Rule {
        id: 28,
...
One obligation is NEVER waived, because it is cheap only in the moment: if you changed the behaviour of an anchored symbol, confirm or reword its claims BEFORE you finish, while the diff is still in your context — `locate {file, symbol}` returns just those claims, a few lines to check. Deferred, the same reconciliation costs a later session thousands of tokens to reconstruct."#,
    ...
    Rule {
        ...
        tags: &["change", "set_change", "rationale", "retag", "unfiled", "resume", "close", "workstream"],
        body: r#"`set_change` selects which CHANGE this session's plan writes belong to, so parallel workstreams stay separable and review/fold can work per task. Pass `rationale` (the task in one sentence, as the dev put it) to OPEN a new change, or `change_id` to RESUME an open one from a prior session (listed in `get_pending`'s `openChanges`). After this, every plan write in this session is tagged to the change automatically; `mark_implemented {change}` folds exactly its entries, and the change closes when its last entry folds — the rationale survives in the history log. `clear: true` detaches (writes go unfiled). `close` (a change id) closes an EMPTY stranded change — refused while it has tagged entries. `retag` (bare ids) with `to` MOVES work that is already pending into another change — a node/group id takes that carrier and everything pending under it, a responsibility/link id takes just that element, a `chg-N` id takes everything filed under it, and "unfiled" takes everything untagged; `to` accepts a change id or "unfiled" and defaults to this session's change. Use it when work landed in the wrong change or one task turns out to be two — never re-write elements just to re-file them. With no arguments, `set_change` reports the current selection and the open changes. The session's selection is per project and in-memory: a fresh session re-selects."#,
    },
    Rule {
        id: 45,
        slug: "sign-off",
        title: "Sign-off snapshots the approved intent; later edits are amendments",
        tags: &["sign-off", "snapshot", "approved", "intent", "amendment", "set_change"],
        body: r#"`set_change {sign_off: true}` (with `change_id`, or alone for the current change) records that the developer approved the plan: it snapshots the change's entries — statement, host, and directives of each — as the approved intent. From then on any claim the agent rewords or adds under the change lands as vagrant (origin `amendment` / `addition`) for the developer's verdict at `mark_implemented` instead of folding, and a dropped signed-off claim is restored for the same verdict ([[fold-after-sign-off]]). The developer's own canvas saves re-stamp the snapshot, so only agent edits count as amendments. `get_pending` and the plan diff mark entries AMENDMENT / ADDITION / DROPPED against the snapshot."#,
    },
    // ---- Evidence. ----
    Rule {
        id: 46,
