...
Statements also carry a markdown-lite display markup the UI renders — and strips for comparison, search, and word-diffing, so it is presentation, never meaning. Wrap the scan anchors — the grammar keywords (including "then") and the response verb — in **bold**, and NOTHING else: "**When** a send-failure status callback arrives, **append** a failed-send event". A ubiquitous claim bolds its leading verb: "**Authenticate** every inbound POST". Never wrap the condition clause or any other run of text in markers — the keyword and its comma already delimit the clause. Markup belongs in responsibility statements ONLY — never in directives, descriptions, names, or properties."#,
    ...
    Rule {
        id: 22,
        title: "Testable claims carry backing tests — construct and attach at the build checkpoint",
        tags: &[
            "test", "tests", "backing", "verify", "verified", "testable", "untested",
            "demonstrate", "verify-map", "verifiedby",
        ],
        body: r#"A claim in a When/While/If form (rule 21) names a concrete trigger, state, or failure — so a test can DEMONSTRATE it mechanically: arrange the condition, assert the response. When you implement such a claim, construct that test in the project's own test suite and attach it in the SAME `mark_implemented` call (`verifiedBy`, alongside `anchors`) — implementing and demonstrating are one checkpoint, not two passes. Health counts the gap deterministically: `testable` (When/While/If claims on code-backed hosts) and `untested` (testable claims with no backing test); drive `untested` toward zero on the scopes you build or reconcile. The test must actually exercise the claim — its trigger arranged, its response asserted; never attach a test that merely touches the same code to clear the counter, and never attach one test to many claims it doesn't demonstrate. A ubiquitous (verb-led) claim may still deserve a test — that stays a judgment call; attach it the same way when you make one (or later via `update_source_map` `verify_entries`). The backing test is a link, never executed by scryer — but it is fingerprint-tripwired like any anchor: when the test's code changes or vanishes, the claim's verify anchor surfaces in health (`verify:{id}`), so a broken demonstration is visible without running anything."#,
    },
];
