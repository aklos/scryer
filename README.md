...
- **Live component previews** — React components and Vue single-file components render deterministically through a per-project Vite dev server, with no agent and no per-component build. Which nodes get a preview is derived from what the server can mount — never a flag stored on the model. Svelte, Solid, and custom elements are not supported yet; Angular is out of scope.
