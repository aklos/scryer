...
description: Set up Claude Code, Codex, Cursor, or any MCP-compatible agent to work with your architecture model.
---
...
Scryer writes two project-local files:

- **`.cursor/mcp.json`** — MCP server configuration for the Cursor CLI (`cursor agent`)
- **`.cursor/cli.json`** — grants non-interactive access only to `Mcp(scryer:*)`

Log in with `cursor agent login` if you haven't already. Scryer also supports
older installations that still expose the standalone `agent` command.
It never launches Cursor with the global `--approve-mcps` switch, so unrelated
MCP servers in the project remain outside Scryer's automated sessions.
...
Then add the least-privilege approval to `.cursor/cli.json`:
