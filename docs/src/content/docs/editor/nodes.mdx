...
Containers have a **technology** field — the framework, language, or platform they run on. On the canvas, a container is marked with a small tab at the top.
...
Components are marked with side ports along their edge, distinguishing them from containers. They should correspond to concrete code units — a class, module, package, or folder in your codebase.
...
4. **Fill with AI** — inside an empty system or container with a linked project, click *Fill with AI* to have an agent scan your codebase and add the children

## Node properties

Every node has:
- **Name** — displayed on the node
- **Description** — what this node *is*, shown on canvas. Edited as a bullet list — one point per line.
- **Notes** — implementation details, deployment context, rationale (not shown on canvas)

Architecture nodes also have:
- **Technology** — framework, language, or platform
