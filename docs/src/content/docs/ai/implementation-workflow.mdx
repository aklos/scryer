...
3. Agent marks the node as <span class="status-pill status-implemented">Implemented</span> via `update_nodes` with a reason
...
Before the loop begins, the agent calls `set_implementing` to pause drift detection — the code it's about to write shouldn't register as architecture drift.
...
Moving to <span class="status-pill status-verified">Verified</span> is a separate step from implementation — don't do it during the build loop. A node is verified when:
