...
## Implementation pauses drift

While an AI agent is actively building from the model, it calls `set_drift_watch` to pause drift detection — otherwise every file it writes would immediately register as drift. Detection resumes automatically when the agent finishes.
...
- **Cursor** — spawned via `cursor agent -p` with legacy `agent -p` fallback (uses your Cursor subscription; MCP from `.cursor/mcp.json`)
