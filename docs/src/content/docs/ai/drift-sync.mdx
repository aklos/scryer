...
While an AI agent is actively building from the model, it calls `set_implementing` to pause drift detection — otherwise every file it writes would immediately register as drift. Detection resumes automatically when the agent finishes.
