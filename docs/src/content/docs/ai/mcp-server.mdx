...
| `validate_model` | Check the model against C4 rules — disconnected nodes, bad edges, mentions without edges |
...
| `set_groups` | Organize nodes into groups (system, container, or component level) |
| `delete_group` | Remove a group — members are ungrouped, not deleted |
| `update_source_map` | Link nodes/flows to source files |
| `set_implementing` | Pause or resume drift detection while implementing |
