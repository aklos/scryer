...
def extract_event_features(df):
    ...
    df["new_session"] = (df["time_diff"] > session_gap) & (
        df["event_type"] == "page_visit"
    )  # ✅ Ensures only `page_visit` starts a new session
    df["session_id"] = df.groupby("visitor_id")["new_session"].cumsum().astype(int)
    df["session_id"] = (
        df["visitor_id"].astype(str) + "_" + df["session_id"].astype(str)
    )  # Unique session ID
    ...
    agg_df = df.groupby("session_id").agg(
        {
            ...
            "created_at": [
                ...
                "max",
                "count",
            ],  # first event, last event, event count
    ...
    agg_df["session_duration"] = (
        agg_df["created_at_max"] - agg_df["created_at_min"]
    ).dt.total_seconds()
...
def cluster_visitors(features):
    ...
    for n in range(
        2, min(10, len(X))
    ):  # Try cluster sizes from 2 to max 10 (or data size)
...
def classify_cohorts(features):
    ...
    non_numeric_columns = ["visitor_id", "path_<lambda>"]
    existing_cols = [col for col in non_numeric_columns if col in features.columns]

    X = features.drop(
        columns=existing_cols, errors="ignore"
    )  # ✅ Drops `path` (raw text) but keeps TF-IDF features
